    ...
    en: {
        ...
        experience: {
            ...
            stena: {
                ...
                bullet4: "Applied Causal Inference Analysis (specifically, Two-Way Fixed Effects DiD model) to evaluate and confirm the benefits of a pricing model."
            },
            chalmers: {
        ...
        projects: {
            ...
            pricingsystem: {
                ...
                bullet1: "<strong><u>Project Background & Objective</u></strong><br>In 2020, Stena Line launched an EMSR-based automatic pricing system to help Regional Managers set ticket prices. The system was not mandatory, which led to very different adoption rates across routes—some routes barely used it (usage ≈ 0%), while others use it often (usage ≈ 100%).<br>Management wanted to assess whether this system actually increased passenger revenue. We selected 15 routes for analysis. However, COVID-19 also broke out in 2020 — at the exact same time the pricing system was introduced — causing dramatic industry-wide revenue fluctuations. This made it impossible to simply compare \"pre-2020 vs. post-2020\" or predict post-pandemic revenue based on historical trends.",
                bullet2: "<strong><u>Methodology / Solution</u></strong><br>We applied a Two-Way Fixed Effects Difference-in-Differences (DID) model:<div style='text-align: center; font-size: 1.1em; margin: 15px 0; padding: 10px; background-color: #f8f9fa; border-left: 4px solid #007bff; border-radius: 4px;'>ln(Revenue<sub>it</sub> + 1) = β ⋅ (UsageRate<sub>it</sub> × Post<sub>t</sub>) + γ<sub>i</sub> + δ<sub>t</sub> + ε<sub>it</sub></div>Where:<br>• <strong>γ<sub>i</sub></strong> (Route Fixed Effects) — controls for time-invariant route-specific characteristics<br>• <strong>δ<sub>t</sub></strong> (Time Fixed Effects) — controls for shocks affecting all routes at the same time<br>• <strong>UsageRate × Post</strong> — the key interaction term capturing post-COVID treatment intensity",
    ...
    zh: {
        ...
        experience: {
            ...
            stena: {
                ...
                bullet4: "应用因果推断分析（具体来说，双向固定效应DiD模型）来评估和确认定价模型的效益。"
            },
            chalmers: {
                title: "研究助理（数据分析师）",
                company: "查尔姆斯理工大学",
        ...
        projects: {
            ...
            pricingsystem: {
                ...
                bullet1: "<strong><u>项目背景与目标</u></strong><br>2020年，Stena Line推出了基于EMSR的自动定价系统，帮助区域经理设定票价。该系统不是强制性的，这导致不同航线的采用率差异很大——有些航线几乎不使用（使用率≈0%），而其他航线经常使用（使用率≈100%）。<br>管理层想要评估这个系统是否真的增加了客运营收。我们选择了15条航线进行分析。然而，COVID-19也在2020年爆发——就在定价系统推出的同一时间——导致了剧烈的行业收入波动。这使得不可能简单地比较\"2020年前vs 2020年后\"或基于历史趋势预测疫情后收入。",
                bullet2: "<strong><u>方法论/解决方案</u></strong><br>我们应用了双向固定效应差分法（DID）模型：<div style='text-align: center; font-size: 1.1em; margin: 15px 0; padding: 10px; background-color: #f8f9fa; border-left: 4px solid #007bff; border-radius: 4px;'>ln(Revenue<sub>it</sub> + 1) = β ⋅ (UsageRate<sub>it</sub> × Post<sub>t</sub>) + γ<sub>i</sub> + δ<sub>t</sub> + ε<sub>it</sub></div>其中：<br>• <strong>γ<sub>i</sub></strong>（航线固定效应）— 控制时间不变的航线特定特征<br>• <strong>δ<sub>t</sub></strong>（时间固定效应）— 控制同时影响所有航线的冲击<br>• <strong>UsageRate × Post</strong> — 捕获疫情后处理强度的关键交互项",
                bullet3: "<strong><u>原因：</u></strong><br>• 定价系统的使用率是连续的（0%-100%）而不是二元的（0/1）。<br>• 没有航线固定效应，传统方法（β * usage_rate）会错误地将航线间的基线收入差异归因于定价系统。γ<sub>i</sub>消除了这些与使用无关的固有跨航线差异。<br>• COVID-19和随后的恢复显著影响了客运营收随时间的变化。时间固定效应δ<sub>t</sub>消除了行业范围的波动，防止错误地将共同冲击归因于定价系统。<br>• 不同航线的收入水平差异很大，使用原始值可能让异常值主导回归。取自然对数稳定方差，允许系数被解释为百分比效应而不是绝对变化。",
                bullet4: "<strong><u>实施步骤</u></strong><br><strong>1. 数据设置</strong><br>• 航线：15条<br>• 时间范围：2015-2025年（月度）<br>• 变量：收入、使用率、时间虚拟变量（post）<br><strong>2. 变量构建</strong><br>• post = 1 如果年份≥2020，否则为0<br>• ur_post = usage_rate × post<br>• 对客运营收应用自然对数变换：ln(Revenue + 1)<br><strong>3. 模型估计</strong><br>• 使用statsmodels OLS与聚类稳健标准误",
                bullet5: "<strong><u>结果</u></strong><br>交互项β估计为0.08，意味着完全采用定价系统的航线将看到约8%的客运营收增长。然而，p值为0.12，表明结果在方向上是积极的，但在常规阈值下只有边际显著性，而不是统计显著性。"
            }
        },
        // Interest Section
        interest: {
    ...
    sv: {
        ...
        experience: {
            ...
            stena: {
                ...
                bullet4: "Tillämpade Causal Inference Analysis (specifikt, Two-Way Fixed Effects DiD-modell) för att utvärdera och bekräfta fördelarna med en prismodell."
            },
            chalmers: {
        ...
        projects: {
            ...
            pricingsystem: {
                ...
                bullet1: "<strong><u>Projektbakgrund & Mål</u></strong><br>2020 lanserade Stena Line ett EMSR-baserat automatiskt prissystem för att hjälpa Regionala Chefer att sätta biljettpriser. Systemet var dock inte obligatoriskt, vilket ledde till mycket olika adoptionsgrader över rutter—vissa rutter använde det knappt (användning ≈ 0%), medan andra använder det ofta (användning ≈ 100%).<br>Ledningen ville bedöma om detta system faktiskt ökade passagerarintäkter. Vi valde 15 rutter för analys. COVID-19 bröt dock också ut 2020 — exakt samtidigt som prissystemet introducerades — vilket orsakade dramatiska branschomfattande intäktsfluktuationer. Detta gjorde det omöjligt att helt enkelt jämföra \"före 2020 vs efter 2020\" eller förutsäga post-pandemiska intäkter baserat på historiska trender.",
                bullet2: "<strong><u>Metodologi / Lösning</u></strong><br>Vi tillämpade en Two-Way Fixed Effects Difference-in-Differences (DID)-modell:<div style='text-align: center; font-size: 1.1em; margin: 15px 0; padding: 10px; background-color: #f8f9fa; border-left: 4px solid #007bff; border-radius: 4px;'>ln(Revenue<sub>it</sub> + 1) = β ⋅ (UsageRate<sub>it</sub> × Post<sub>t</sub>) + γ<sub>i</sub> + δ<sub>t</sub> + ε<sub>it</sub></div>Där:<br>• <strong>γ<sub>i</sub></strong> (Rutt Fast Effekter) — kontrollerar för tidsinvarianta rutt-specifika egenskaper<br>• <strong>δ<sub>t</sub></strong> (Tid Fast Effekter) — kontrollerar för chocker som påverkar alla rutter samtidigt<br>• <strong>UsageRate × Post</strong> — nyckelinteraktionstermen som fångar post-COVID behandlingsintensitet",
    ...
    de: {
        ...
        experience: {
            ...
            stena: {
                ...
                bullet4: "Wendete Causal Inference Analysis (spezifisch, Two-Way Fixed Effects DiD-Modell) an, um die Vorteile eines Preismodells zu bewerten und zu bestätigen."
            },
            chalmers: {
        ...
        projects: {
            ...
            pricingsystem: {
                ...
                bullet1: "<strong><u>Projekt-Hintergrund & Ziel</u></strong><br>2020 führte Stena Line ein EMSR-basiertes automatisches Preissystem ein, um Regionalmanagern bei der Festlegung von Ticketpreisen zu helfen. Das System war jedoch nicht verpflichtend, was zu sehr unterschiedlichen Adoptionsraten über Routen führte—einige Routen nutzten es kaum (Nutzung ≈ 0%), während andere es oft nutzen (Nutzung ≈ 100%).<br>Das Management wollte bewerten, ob dieses System tatsächlich die Passagierumsätze erhöhte. Wir wählten 15 Routen für die Analyse aus. COVID-19 brach jedoch auch 2020 aus — genau zur gleichen Zeit, als das Preissystem eingeführt wurde — was dramatische branchenweite Umsatzschwankungen verursachte. Dies machte es unmöglich, einfach \"vor 2020 vs. nach 2020\" zu vergleichen oder Post-Pandemie-Umsätze basierend auf historischen Trends vorherzusagen.",
                bullet2: "<strong><u>Methodik / Lösung</u></strong><br>Wir wendeten ein Two-Way Fixed Effects Difference-in-Differences (DID)-Modell an:<div style='text-align: center; font-size: 1.1em; margin: 15px 0; padding: 10px; background-color: #f8f9fa; border-left: 4px solid #007bff; border-radius: 4px;'>ln(Revenue<sub>it</sub> + 1) = β ⋅ (UsageRate<sub>it</sub> × Post<sub>t</sub>) + γ<sub>i</sub> + δ<sub>t</sub> + ε<sub>it</sub></div>Wobei:<br>• <strong>γ<sub>i</sub></strong> (Route Fixed Effects) — zeitinvariante routenspezifische Eigenschaften kontrolliert<br>• <strong>δ<sub>t</sub></strong> (Time Fixed Effects) — Schocks kontrolliert, die alle Routen gleichzeitig betreffen<br>• <strong>UsageRate × Post</strong> — der Schlüsselinteraktionsterm, der Post-COVID-Behandlungsintensität erfasst",
    ...
    nl: {
        ...
        experience: {
            ...
            stena: {
                ...
                bullet4: "Toegepaste Causal Inference Analysis (specifiek, Two-Way Fixed Effects DiD-model) om de voordelen van een prijsmodel te evalueren en bevestigen."
            },
            chalmers: {
        ...
        projects: {
            ...
            pricingsystem: {
                ...
                bullet1: "<strong><u>Project Achtergrond & Doel</u></strong><br>In 2020 lanceerde Stena Line een EMSR-gebaseerd automatisch prijssysteem om Regionale Managers te helpen bij het instellen van ticketprijzen. Het systeem was echter niet verplicht, wat leidde tot zeer verschillende adoptiepercentages over routes—sommige routes gebruikten het nauwelijks (gebruik ≈ 0%), terwijl anderen het vaak gebruiken (gebruik ≈ 100%).<br>Het management wilde beoordelen of dit systeem daadwerkelijk de passagiersomzet verhoogde. We selecteerden 15 routes voor analyse. COVID-19 brak echter ook uit in 2020 — precies op hetzelfde moment dat het prijssysteem werd geïntroduceerd — wat dramatische branche-brede omzetfluctuaties veroorzaakte. Dit maakte het onmogelijk om eenvoudig \"pre-2020 vs. post-2020\" te vergelijken of post-pandemische omzet te voorspellen op basis van historische trends.",
                bullet2: "<strong><u>Methodologie / Oplossing</u></strong><br>We pasten een Two-Way Fixed Effects Difference-in-Differences (DID)-model toe:<div style='text-align: center; font-size: 1.1em; margin: 15px 0; padding: 10px; background-color: #f8f9fa; border-left: 4px solid #007bff; border-radius: 4px;'>ln(Revenue<sub>it</sub> + 1) = β ⋅ (UsageRate<sub>it</sub> × Post<sub>t</sub>) + γ<sub>i</sub> + δ<sub>t</sub> + ε<sub>it</sub></div>Waar:<br>• <strong>γ<sub>i</sub></strong> (Route Fixed Effects) — controleert voor tijd-invariante route-specifieke kenmerken<br>• <strong>δ<sub>t</sub></strong> (Time Fixed Effects) — controleert voor schokken die alle routes tegelijk beïnvloeden<br>• <strong>UsageRate × Post</strong> — de sleutelinteractieterm die post-COVID behandelingsintensiteit vastlegt",
