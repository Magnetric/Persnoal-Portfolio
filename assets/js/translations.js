...
const translations = {
    ...
    zh: {
        ...
        projects: {
            ...
            pricingsystem: {
                title: "使用双向固定效应DID模型评估定价系统对收入的影响",
                description: "使用先进的计量经济学方法评估了基于EMSR的自动定价系统对乘客收入的影响。分析通过双向固定效应差分法解决了将定价系统效果与并发COVID-19影响分离的挑战。",
                bullet1: "<strong><u>项目背景与目标</u></strong><br>2020年，公司推出了基于EMSR的自动定价系统，帮助区域经理设定票价。然而，该功能不是强制性的，这导致不同路线的采用率差异很大——有些路线几乎不使用它（使用率≈0%），而其他路线则持续高水平采用（使用率≈100%）。<br>管理层想要评估这个系统是否真的增加了乘客收入。我们选择了15条路线进行分析。然而，COVID-19也在2020年爆发——就在定价系统推出的同一时间——导致了戏剧性的行业收入波动。这使得不可能简单地比较\"2020年前vs 2020年后\"或基于历史趋势预测疫情后收入。",
                ...
                bullet3: "<strong><u>原因：</u></strong><br>• 定价系统的采用是连续的（0%-100%）而不是二元的（0/1）。<br>• 没有路线固定效应，传统规范（β * usage_rate）会错误地将路线间的基线收入差异归因于定价系统。γ<sub>i</sub>消除了这些与使用无关的固有跨路线差异。<br>• COVID-19和随后的恢复显著影响了乘客收入随时间的变化。时间固定效应δ<sub>t</sub>消除了行业范围的波动，防止错误地将共同冲击归因于定价系统。<br>• 不同路线的收入水平差异很大，使用原始值可能让异常值主导回归。取自然对数稳定方差，允许系数被解释为百分比效应而不是绝对变化。",
                bullet4: "<strong><u>实施步骤</u></strong><br><strong>1. 数据设置</strong><br>• 路线：15条<br>• 时间范围：2015-2025年（月度）<br>• 变量：收入、使用率、时间虚拟变量（post）<br><strong>2. 变量构建</strong><br>• post = 1 如果年份≥2020，否则为0<br>• ur_post = usage_rate × post<br>• 对收入应用自然对数变换：ln(Revenue + 1)<br><strong>3. 模型估计</strong><br>• 使用statsmodels OLS与聚类稳健标准误",
                bullet5: "<strong><u>结果</u></strong><br>交互项β估计为0.08，意味着完全采用定价系统的路线将看到约8%的乘客收入增长。然而，p值为0.12，表明结果在方向上是积极的，但在常规阈值下只有边际显著性，而不是统计显著性。"
            }
        },
        // Interest Section
        interest: {
